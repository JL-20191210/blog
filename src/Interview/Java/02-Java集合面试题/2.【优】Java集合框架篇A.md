...
![面渣逆袭集合框架篇封面图](assets/collection-mianzhanixi-collection1-1756780078635198.jpg)
...
![面渣逆袭集合框架篇.pdf第二版](assets/collection-20250108182441-1756780078635200.png)
...
![面渣逆袭已经提交 1438 次 GitHub 记录](assets/javase-20241223085158.png)
...
![面渣逆袭集合框架篇.pdf暗黑版](assets/collection-20250108182547.png)
...
![李豪：优先级队列](assets/PriorityQueue-8dca2f55-a7c7-49e1-95a5-df1a34f2aef5.png)

双端队列 ArrayDeque 也实现了 Queue 接口，是一个基于数组的，可以在两端插入和删除元素的队列。

![李豪：双端队列](assets/arraydeque-1e7086a3-3d31-4553-aa16-5eaf2193649e.png)

LinkedList 实现了 Queue 接口的子类 Deque，所以也可以当做双端队列来使用。

![二哥的 Java 进阶之路](assets/list-war-2-02.png)
...
![疯狂的技术宅：队列](assets/collection-20240412224341.png)

栈是一种后进先出（LIFO, Last-In-First-Out）的数据结构，最后一个加入栈的元素会成为第一个被移除的元素。

![Wang Wei：栈](assets/collection-20240412224549.png)
...
![二哥的 Java 进阶之路：Collection源码](assets/collection-20240711092853.png)
...
![三分恶面渣逆袭：ArrayList和LinkedList的数据结构](assets/collection-2.png)
...
![三分恶面渣逆袭：ArrayList和LinkedList中间插入](assets/collection-3.png)
...
![三分恶面渣逆袭：ArrayList和LinkedList中间删除](assets/collection-4.png)

#### ArrayList 和 LinkedList 是否支持随机访问？

①、ArrayList 是基于数组的，也实现了 RandomAccess 接口，所以它支持随机访问，可以通过下标直接获取元素。

![二哥的 Java 进阶之路：ArrayList](assets/collection-20240319092907.png)

②、LinkedList 是基于链表的，所以它没法根据下标直接获取元素，不支持随机访问。

![二哥的 Java 进阶之路：LinkedList](assets/collection-20240319093038.png)

#### ArrayList 和 LinkedList 内存占用有何不同？

ArrayList 是基于数组的，是一块连续的内存空间，所以它的内存占用是比较紧凑的；但如果涉及到扩容，就会重新分配内存，空间是原来的 1.5 倍。

![二哥的 Java 进阶之路：ArrayList的扩容](assets/collection-20240319093453.png)
...
![数组和链表的内存占用区别](assets/collection-20241011102136.png)
...
![二哥的 Java 进阶之路：elementData](assets/collection-20250106155608.png)
...
![二哥的 Java 进阶之路：Vector源码](assets/collection-20240619110254.png)
...
![三分恶面渣逆袭：CopyOnWriteArrayList原理](assets/collection-7.png)
...
HashMap put 流程大致分为 5 步：

1. **计算 key 的 hash 值，并定位到数组的索引位置。**
2. **如果桶为空，直接插入。**
3. **如果桶不为空，判断 key 是否已存在：存在就覆盖，否则挂到链表或红黑树上。**
4. **当链表长度超过 8并且数组的长度要大于64 ，会转成红黑树，提高查询效率。**
5. **插入完成后，如果元素个数超过阈值（容量 * 负载因子），就会触发扩容，重新计算节点位置。**
...
![三分恶面渣逆袭：HashMap查找流程图](assets/collection-14.png)
...
![二哥的 Java 进阶之路：JDK 8中的 hash 函数](assets/collection-20240325100934.png)

以初始长度 16 为例，16-1=15。2 进制表示是`0000 0000 0000 0000 0000 0000 0000 1111`。只取最后 4 位相等于哈希值的高位都丢弃了。

![三分恶面渣逆袭：哈希&运算](assets/collection-15.png)
...
![三分恶面渣逆袭：hash 函数示意图](assets/collection-16.jpg)
...
**HashMap 的容量必须是 2 的幂，主要有两个原因：**

1. **高效取模：用 `(n-1) & hash` 代替 `% n`，计算更快。**
2. **减少冲突：当 n 是 2 的幂时，`n-1` 的二进制低位全是 1，可以让 hash 值的所有低位都参与计算，保证元素分布均匀。**

**所以 HashMap 会强制把容量调整为 2 的幂**
...
![三分恶面渣逆袭：拉链法 VS 开放地址法](assets/collection-20.png)
...
![二哥的 Java 进阶之路：HashMap 扩容](assets/collection-20240323113620.png)
...
![二哥的 Java 进阶之路：为什么选择 0.75](assets/collection-20250108101417.png)
...
![二哥的 Java 进阶之路：JDK7 扩容前](assets/hashmap-resize-01.png)
...
![二哥的 Java 进阶之路： JDK7扩容后](assets/hashmap-resize-02.png)
...
![三分恶面渣逆袭：扩容位置变化](assets/collection-26.png)

这样可以避免重新计算所有元素的哈希值，只需检查高位的某一位，就可以快速确定新位置。

![三分恶面渣逆袭：扩容节点迁移示意图](assets/collection-27.png)
...
![二哥的 Java 进阶之路：JDK7 JDK8 扩容时机的不同](assets/collection-20250108174154.png)


④、哈希扰动算法也进行了优化。JDK 7 是通过多次移位和异或运算来实现的。

![二哥的 Java 进阶之路：JDK 7 的 hash 方法](assets/collection-20240512093223.png)

JDK 8 让 hash 值的高 16 位和低 16 位进行了异或运算，让高位的信息也能参与到低位的计算中，这样可以极大程度上减少哈希碰撞。

![二哥的 Java 进阶之路：JDK 8 的 hash 方法](assets/collection-20240512093327.png)
...
![三分恶面渣逆袭：自定义HashMap整体结构](assets/collection-29.png)
...
[HashMap 详解](https://javabetter.cn/collection/hashmap.html#_04%E3%80%81%E7%BA%BF%E7%A8%8B%E4%B8%8D%E5%AE%89%E5%85%A8)

HashMap 不是线程安全的，主要有以下几个问题：

①、多线程下扩容会死循环。JDK7 中的 HashMap 使用的是头插法来处理链表，在多线程环境下扩容会出现环形链表，造成死循环。

![二哥的 Java 进阶之路：环形链表](assets/hashmap-thread-nosafe-07.png)
...
![二哥的 Java 进阶之路](assets/hashmap-thread-nosafe-10.png)

③、put 和 get 并发时，可能导致 get 为 null。线程 1 执行 put 时，因为元素个数超出阈值而扩容，线程 2 此时执行 get，就有可能出现这个问题。

![二哥的 Java 进阶之路：get 到 null](assets/collection-20240326085630.png)
...
在早期的 JDK 版本中，可以用 Hashtable 来保证线程安全。

Hashtable 在方法上加了 [synchronized 关键字](https://javabetter.cn/thread/synchronized-1.html)。

![二哥的 Java 进阶之路：Hashtable](assets/collection-20240323125211.png)

另外，可以通过 `Collections.synchronizedMap` 方法返回一个线程安全的 Map，内部是通过 synchronized 对象锁来保证线程安全的，比在方法上直接加 synchronized 关键字更轻量级。

![二哥的 Java 进阶之路：Collections.synchronizedMap](assets/collection-20240323125418.png)
...
![初念初恋：ConcurrentHashMap 8 中的实现](assets/map-20230816155924.png)
...
![三分恶面渣逆袭：Entry节点](assets/collection-33.png)

从而实现插入的顺序或访问顺序。

![三分恶面渣逆袭：LinkedHashMap实现原理](assets/collection-34.png)
...
![二哥的 Java 进阶之路：TreeMap源码](assets/collection-20240330124711.png)
...
![三分恶面渣逆袭：HashSet套娃](assets/collection-36.png)
...
![这是我在牛客上看到的](assets/javase-20241230165717.png)

![我觉得都是蛮中肯的评价](assets/javase-20241230165749.png)

![双非硕测开对二哥八股的认可](assets/collection-20250108181632.png)
...
![面渣逆袭集合框架篇：epub 版本](assets/collection-20250108182335.png)

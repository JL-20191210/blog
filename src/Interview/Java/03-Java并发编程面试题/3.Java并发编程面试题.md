...
CLH：Craig、Landin and Hagersten队列，是单向链表，AQS中的队列是CLH变体的虚拟双向队列（FIFO），AQS是通过将每条请求共享资源的线程封装成一个节点来实现锁的分配。主要原理图如下：
...
![img](assets\1751856711304-a2fb09a3-bcca-4c91-b99a-40a2158eaf1e.webp)从内存结构图，我们可以看到：
...
![img](assets\1751876795536-3ef80944-c6b0-413b-bb4b-8dd5f38f862e.webp)
...
![img](assets\1751876922588-dd0e26ba-f7d3-4838-810b-6d1fb316074d.webp)
...
当线程池的任务队列已满，且线程池中的线程数也达到了最大值时，线程池会执行指定的**拒绝策略（`RejectedExecutionHandler`）**来处理新提交的任务。

Java 提供了**四种内置拒绝策略**，还支持自定义：

✅ 四种常见拒绝策略（位于 `java.util.concurrent.ThreadPoolExecutor`）：

1. **`AbortPolicy`（默认）**
   - 直接抛出 `RejectedExecutionException` 异常，阻止系统继续提交任务；
   - **适合需要强制关注拒绝事件的场景**。
2. **`CallerRunsPolicy`**
   - 由提交任务的线程（调用 `execute()` 的线程）自己执行该任务；
   - **可以减缓任务提交速度，避免任务丢失**，但会影响调用者线程性能。
3. **`DiscardPolicy`**
   - 直接丢弃当前被拒绝的任务，**不抛出异常、不记录日志**；
   - **适用于可以容忍少量任务丢失的场景**。
...
可以通过实现 `RejectedExecutionHandler` 接口，编写自己的拒绝逻辑，例如：

```
java复制编辑executor.setRejectedExecutionHandler((r, executor) -> {
    // 可以记录日志、发送报警、将任务转移到其他线程池等
...
可以，当核心线程数为0的时候，会创建一个非核心线程进行执行。从下面的源码也可以看到，当核心线程数为 0 时，来了一个任务之后，会先将任务添加到任务队列，同时也会判断当前工作的线程数是否为 0，如果为 0，则会创建线程来执行线程池的任务。

![img](assets\1751881452560-723ac320-853a-4996-b545-ef22d7ed4167.webp)
...
《阿里巴巴 Java 开发手册》中提到，禁止使用这些方法来创建线程池，而应该手动 new ThreadPoolExecutor 来创建线程池。这一条规则的背后，是大量血淋淋的生产事故，最典型的就是 newFixedThreadPool 和 newCachedThreadPool，可能因为资源耗尽导致 OOM 问题。所以，不建议使用 Executors 提供的两种快捷的线程池，原因如下：
...
> 这两种方法都是用来关闭线程池，但它们的行为和作用场景有所不同。

#### **1**. shutdown()

**作用：平滑关闭线程池**

- 调用后，线程池状态变为 `SHUTDOWN`（非立即终止状态）
- 不接受新任务，继续执行已提交（正在执行和队列中等待）的任务
- 线程池会在所有任务执行完毕后，才真正关闭
- 线程池空闲线程会被中断（`interruptIdleWorkers()`），但正在运行的线程不会被强制中断
- 如果此时继续提交任务，会抛出 `RejectedExecutionException`

**源码关键点：**

```java
advanceRunState(SHUTDOWN);           // 状态变为SHUTDOWN，拒绝新任务
interruptIdleWorkers();              // 中断空闲线程（比如阻塞在等待任务的线程）
onShutdown();                       // 线程池关闭时的扩展操作（如钩子）
tryTerminate();                     // 尝试终止线程池
```

**总结：**

- 是一种“有序关机”，允许已经提交的任务执行完毕
- 不会强制中断正在执行的任务线程，保证任务完整性
...
- 状态变为 `STOP`（立即终止状态）
- 不接受新任务，也不执行队列中等待的任务
- 试图中断所有正在执行的线程（`interruptWorkers()`）
- 返回尚未执行的任务列表（`drainQueue()` 将任务从队列中取出返回）
- 但中断只能对处于阻塞、等待、sleep 等状态的线程有效，无法强制停止CPU密集型或未响应中断的线程
- 因此，`shutdownNow()` 并不保证线程池立即关闭，仍可能需要等待部分任务完成后才真正终止

**源码关键点：**

```java
advanceRunState(STOP);               // 状态变为STOP，立即停止
interruptWorkers();                 // 中断所有工作线程（包括正在执行的）
tasks = drainQueue();               // 清空队列，返回未执行任务
tryTerminate();                    // 尝试终止线程池
...
- 是一种“强制关机”，试图停止所有活动任务并清空队列
- 但依赖线程对中断响应，不能保证线程池立即停止
...
| 线程池状态       | SHUTDOWN                 | STOP                                 |
| 是否接受新任务   | 不接受                   | 不接受                               |
| 队列中任务执行   | 继续执行                 | 不执行，返回未执行任务列表           |
| 正在执行线程行为 | 等待任务执行完成，不中断 | 试图中断线程，线程若不响应中断则继续 |
| 是否立即关闭     | 否，等待任务完成后关闭   | 尽快关闭，但不保证立即               |
| 返回值           | 无                       | 返回未执行任务列表                   |

**使用建议**

- **正常业务关闭时**，优先调用 `shutdown()`，保证任务执行完整
- **紧急情况或快速关闭时**，调用 `shutdownNow()`，并做好线程中断的响应处理（在任务中定期检查中断状态）
- **线程任务设计时**，尽量支持响应中断（如用`Thread.sleep()`、阻塞队列、`wait()`等可中断API
...
>
> - 触发线程池**平滑关闭**，不再接收新任务。
> - 会等待**线程池中已提交的任务执行完成**，包括正在执行和队列中的任务。
> - 会中断**空闲线程**（等待任务的线程），但不会中断正在执行任务的线程。
> - 如果再提交任务会抛出 `RejectedExecutionException`。
>
> **2. `shutdownNow()` 方法作用：**
>
> - 触发线程池**立即关闭**，不再接收新任务。
> - 会尝试**中断所有正在执行的线程**，并返回还未执行的任务列表。
> - 不保证线程池立刻关闭，线程是否能停止取决于线程是否响应中断。
> - 会清空任务队列，未执行的任务会被返回。
>
> **3. 底层实现区别：**
>
> - `shutdown()` 将线程池状态设置为 `SHUTDOWN`，只中断空闲线程。
> - `shutdownNow()` 将状态设置为 `STOP`，中断所有线程，并清空任务队列。
>
> **4. 使用场景：**
>
> - 正常业务结束时调用 `shutdown()`，保证任务执行完整。
> - 需要快速终止线程池时调用 `shutdownNow()`，需确保任务支持中断响应。

#### 9.提交给线程池中的任务可以被撤回吗？

可以，当向线程池提交任务时，会得到一个`Future`对象。这个`Future`对象提供了几种方法来管理任务的执行，包括取消任务。取消任务的主要方法是`Future`接口中的`cancel(boolean mayInterruptIfRunning)`方法。这个方法尝试取消执行的任务。参数`mayInterruptIfRunning`指示是否允许中断正在执行的任务。如果设置为`true`，则表示如果任务已经开始执行，那么允许中断任务；如果设置为`false`，任务已经开始执行则不会被中断。
...
`synchronized` 关键字的作用用于确保在多线程环境下，只有一个线程能够进入同步块（这里是 `synchronized (Singleton.class)`）。在创建单例对象时，通过 `synchronized` 保证了创建过程的线程安全性，避免多个线程同时创建多个单例对象。`volatile` 确保了对象引用的可见性和创建过程的有序性，避免了由于指令重排序而导致的错误。`instance = new Singleton();` 这行代码并不是一个原子操作，它实际上可以分解为以下几个步骤：

- 分配内存空间。
- 实例化对象。
- 将对象引用赋值给 `instance`。

由于 Java 内存模型允许编译器和处理器对指令进行重排序，在没有 `volatile` 的情况下，可能会出现重排序，例如先将对象引用赋值给 `instance`，但对象的实例化操作尚未完成。这样，其他线程在检查 `instance == null` 时，会认为单例已经创建，从而得到一个未完全初始化的对象，导致错误。`volatile` 可以保证变量的可见性和禁止指令重排序。它确保对 `instance` 的修改对所有线程都是可见的，并且保证了上述三个步骤按顺序执行，避免了在单例创建过程中因指令重排序而导致的问题。

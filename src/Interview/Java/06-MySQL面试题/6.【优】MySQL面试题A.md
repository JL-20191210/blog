...
![二哥编程星球的球友：小赢科技 OC了](assets/mysql-20250227084922.png)
...
![三分恶面渣逆袭：数据库三范式](assets/mysql-16e74a6b-a42a-464e-9b10-0252ee7ecc6e.jpg)
...
![Ruthless：订单商品关联表](assets/mysql-20240418093726.png)
...
![二哥的 Java 进阶之路：修改t1主键](assets/mysql-20250305181907.png)
...
![二哥的 Java 进阶之路：MySQL 8.0 官方手册](assets/mysql-20250305183220.png)
...
![球友拿到了咪咕的大模型开发实习](assets/mysql-20250305183751.png)
...
![二哥的 Java 进阶之路：MySQL常用命令](assets/mysql-20240313093551.png)
...
![二哥的 Java 进阶之路：隐式转换](assets/mysql-20240425111246.png)
...
![二哥的 Java 进阶之路：SQL 执行](assets/mysql-20240415102041.png)
...
![不要迷恋发哥：段、区、页、行](assets/mysql-20240515110034.png)
...
![二哥的 Java 进阶之路：存储引擎](assets/mysql-20240408073338.png)
...
![MySQL 官网：innodb-limits.html](assets/mysql-20240408074630.png)
...
![WindWant：InnoDB 存储引擎整体架构图](assets/mysql-20250404111739.png)
...
![nekolr's blog：数据页结构](assets/mysql-20250404113446.png)
...
![二哥的 Java 进阶之路：对 buffer pool 进行调整](assets/mysql-20250312093325.png)
...
![球友报喜字节二面过了](assets/mysql-20250313170334.png)
...
![greatsql 社区：redo log 恢复](assets/mysql-20250314160157.png)
...
![greatsql社区：redo log 文件组](assets/mysql-20250314161331.png)
...
![三分恶面渣逆袭：redo log 缓冲](assets/mysql-e1f59341-0695-45db-b759-30db73314e39.jpg)
...
![酷酷博客园：Page Cache](assets/mysql-20250317160220.png)
...
![西维蜀黍：Redo Log 工作原理](assets/mysql-20250318155855.png)
...
![梦里花。：回表](assets/mysql-20250322095940.png)
...
![球友进入腾讯云的 HR 面了](assets/mysql-20250322103527.png)
...
![二哥的 Java 进阶之路：子查询的执行计划](assets/mysql-20250323213511.png)
...
![二哥的 Java 进阶之路：JOIN 的执行计划](assets/mysql-20250323213756.png)
...
![球友对星球的一种褒奖](assets/mysql-20250323214034.png)

#### JOIN操作为什么要小表驱动大表？

第一，如果大表的 JOIN 字段有索引，那么小表的每一行都可以通过索引快速匹配大表。

![极客时间：Index Nested-Loop Join](assets/mysql-20250324121435.png)
...
![极客时间：Block Nested-Loop Join](assets/mysql-20250324121526.png)

时间复杂度为小表分段数 K 乘以大表行数 M，其中 K = 小表行数 N / 内存大小 join_buffer_size。

![二哥的 Java 进阶之路：join_buffer_size](assets/mysql-20250324152505.png)

显然小表做驱动表的时候 K 的值更小，大表做驱动表的时候需要多次分段。

![贴一段球友对这个问题的分析](assets/mysql-20250326213442.png)
...
![二哥的 Java 进阶之路：join 的验证，创建两个表](assets/mysql-20250324155729.png)
...
![二哥的 Java 进阶之路：join 的执行计划](assets/mysql-20250324155453.png)
...
![阿里开发手册：MySQL 规约之 JOIN 多表](assets/mysql-20250324163735.png)

memo：2025 年 3 月 24 日修改至此，今天[有球友反馈说](https://javabetter.cn/zhishixingqiu/)，简历上加了 mydb 项目后，也顺利拿到腾讯的暑期实习 offer。

![球友拿到腾讯的暑期 offer+1](assets/mysql-20250324163940.png)
...
![二哥的 Java 进阶之路：sort 的一些参数](assets/mysql-20250326213740.png)
...
![二哥的 Java 进阶之路：filesort](assets/mysql-20250326220036.png)

----这部分是帮助大家理解 start，面试中可不背----

好，让我们来验证一下 filesort 的情况，建表、插入数据。

![二哥的 Java 进阶之路：验证 filesort](assets/mysql-20250326220632.png)

执行 explain 查看执行计划。

![二哥的 Java 进阶之路：filesort 的执行计划](assets/mysql-20250326220857.png)
...
![球友拿到阿里云的意向书了](assets/mysql-%E9%98%BF%E9%87%8C%E4%BA%91%E7%BB%99%E6%84%8F%E5%90%91%E4%B9%A6%E4%BA%86.png)

#### 全字段排序和 rowid 排序了解多少？

当排序字段是索引字段且满足最左前缀原则时，MySQL 可以直接利用索引的有序性完成排序。

![二哥的 Java 进阶之路：索引排序](assets/mysql-20250327164223.png)

当无法使用索引排序时，MySQL 需要在内存或磁盘中进行排序操作，分为全字段排序和 rowid 排序两种算法。

![极客时间：全字段排序](assets/mysql-20250327172128.png)
...
![极客时间：rowid 排序](assets/mysql-20250327171803.png)
...
![二哥的 Java 进阶之路：Sort_merge_passes](assets/mysql-20250327173629.png)
...
![球友字节和腾讯都 OC了](assets/mysql-20250328155527.png)
...
![三分恶面渣逆袭：EXPLAIN](assets/mysql-e234658f-5672-4a8d-9a75-872b305a171d.jpg)
...
![二哥的 Java 进阶之路：explain json 格式](assets/mysql-20250329103010.png)
...
![二哥的 Java 进阶之路：explain 结果](assets/mysql-20240417092646.png)
...
![球友美团 offer 口头oc 了](assets/mysql-20250329111600.png)
...
![三分恶面渣逆袭：索引加快查询远离](assets/mysql-6b9c9901-9bf3-46ed-a5c4-c1b781965c1e.jpg)

它一般是 B+ 树结构，查找效率是 O(log n)，比从头到尾扫一遍数据要快得多。

![MySQL 索引](assets/mysql-20250330123726.png)
...
![二哥的 Java 进阶之路：有索引和没有索引的查询效率](assets/mysql-20250330124012.png)

创建数据库和表。

![二哥的 Java 进阶之路：创建 index 的验证表](assets/mysql-20250330124209.png)

插入 10 万条数据。

![二哥的 Java 进阶之路：插入数据](assets/mysql-20250330124250.png)

然后依次执行 explain 查看没有索引和有索引时的执行计划。

![二哥的 Java 进阶之路：对比有索引和没有索引的差别](assets/mysql-20250330124502.png)
...
![球友拿到了淘天的暑期实习](assets/mysql-20250330170053.png)

### 36.🌟能简单说一下索引的分类吗？

从功能上分类的话，有主键索引、唯一索引、全文索引；从数据结构上分类的话，有 B+ 树索引、哈希索引；从存储内容上分类的话，有聚簇索引、非聚簇索引。

![二哥的 Java 进阶之路：索引类型](assets/mysql-20240311225809.png)

#### 你对主键索引了解多少？

主键索引用于唯一标识表中的每条记录，其列值必须唯一且非空。创建主键时，MySQL 会自动生成对应的唯一索引。

![processon：主键索引](assets/mysql-20250331165636.png)
...
![二哥的 Java 进阶之路：MySQL 官方文档隐藏主键](assets/mysql-20250331165053.png)

可以通过 `show index from table_name` 查看索引信息：

![二哥的 Java 进阶之路：索引信息](assets/mysql-20240312090221.png)
...
![二哥的 Java 进阶之路：主键索引和唯一索引](assets/mysql-20250331171518.png)
...
![二哥的 Java 进阶之路：unique key 和 unique index](assets/mysql-20250331174044.png)
...
![球友拿到了淘天的暑期实习 offer](assets/mysql-%E5%8E%89%E5%AE%B3%E7%9A%84%EF%BC%8C%E6%81%AD%E5%96%9C%E5%95%8A.png)
...
![二哥编程星球的球友拿到美团实习 offer 了](assets/mysql-20250401155231.png)
...
![二哥的 Java 进阶之路：小表的全表扫描比索引会更快](assets/mysql-20250402144634.png)

得出的结论的确是这样的，全表扫描更快一些。

![二哥的 Java 进阶之路：小表在索引和全表扫描时的结果](assets/mysql-20250402144804.png)
...
一般来说，主键、唯一键、以及经常作为查询条件的字段最适合加索引。

除此之外，字段的区分度要高，这样索引才能起到过滤作用；

如果字段经常用于表连接、排序或分组，也建议加索引。

同时如果多个字段经常一起出现在查询条件中，也可以建立联合索引来提升性能。
...
![球友拿到了百度的实习 offer](assets/mysql-20250402160141.png)
...
![用户1260737：B+树](assets/mysql-20240322142950.png)
...
![William Johnson：b+树](assets/mysql-20241104203402.png)
...
![二哥的 Java 进阶之路：B 树](assets/mysql-20240322132606.png)

再来说说内存和磁盘的 IO 读写。

![二哥的 Java 进阶之路：IO 读写](assets/mysql-20240322133650.png)
...
![用户1260737：二叉树](assets/mysql-20240322140825.png)

B 树的节点通常与页的大小对齐，这样每次从磁盘加载一个节点时，正好就是一页的大小。

![用户1260737：B 树](assets/mysql-20240322141957.png)
...
![死磕 Java：B+树](assets/mysql-20250403113413.png)
...
![Brand博客园：B+树](assets/mysql-20250403114455.png)
...
![球友收到美团 offer 了](assets/mysql-20250403120541.png)
...
![清幽之地：B+树存储数据条数](assets/mysql-16f3523d-20b0-4376-908d-ac40b329768f.jpg)
...
![二哥的 Java 进阶之路：GitHub源码](assets/mysql-20250404125013.png)
...
![yifanSJ：3 层 B+树](assets/mysql-20250404105416.png)
...
![球友问英文版的面炸逆袭](assets/mysql-%E8%AF%B7%E9%97%AE%E6%9C%89%E8%8B%B1%E6%96%87%E7%BF%BB%E8%AF%91%E7%89%88%E7%9A%84%E5%90%97.png)

### 43.索引为什么用 B+树不用普通二叉树？

普通二叉树的每个节点最多有两个子节点。当数据按顺序递增插入时，二叉树会退化成链表，导致树的高度等于数据量。

![二哥的Java 进阶之路：普通二叉树](assets/mysql-20241115151059.png)
...
![二哥的Java 进阶之路：AVL 树](assets/mysql-20241115151729.png)
...
![极客时间：B 树](assets/mysql-20240325115614.png)
...
![极客时间：B+树](assets/mysql-20240325115641.png)
...
![dunwu：跳表](assets/mysql-20250405105517.png)
...
![oi-wiki：查找 45](assets/mysql-20241223114806.png)

第一步，从根节点开始，因为比 25 大，所以从右子树开始。因为 45 比 35大，所以和右边的索引比较，右侧的索引也是 45，所以继续往右子树查找。

![oi-wiki：从根节点开始](assets/mysql-20241223114907.png)

第二步，从叶子节点 45 开始，依次遍历，找到 45。

![oi-wiki：找到 45](assets/mysql-20241223115300.png)
...
![维基百科：快速排序](assets/mysql-Sorting_quicksort_anim.gif)
...
![一颗剽悍的种子：B+树的结构](assets/mysql-20240312092745.png)

Hash 索引只支持等值查找，速度快但功能弱，常见于 Memory 引擎。

![业余码农：哈希索引](assets/mysql-20240312094537.png)
...
![二哥的 Java 进阶之路](assets/mysql-20240312095811.png)
...
InnoDB 的主键索引就是聚簇索引，叶子节点不仅存储了主键值，还存储了其他列的值，因此按照主键进行查询的速度会非常快。

![代码敲上天.：聚簇索引](assets/mysql-20240311231652.png)
...
![代码敲上天.非聚簇索引，以 age 为索引](assets/mysql-20240311231611.png)
...
![球友夸二哥的简历修改的好](assets/mysql-20250405114505.png)

### 47.🌟回表了解吗？

当使用非聚簇索引进行查询时，MySQL 需要先通过非聚簇索引找到主键值，然后再根据主键值回到聚簇索引中查找完整数据行，这个过程称为回表。

![梦里花。：InnoDB 回表](assets/mysql-20250406120133.png)
...
![Brand：回表](assets/mysql-20250408110030.png)
...
![极客时间：MRR](assets/mysql-20250406125740.png)
...
![二哥的 Java 进阶之路：MRR](assets/mysql-20250406121543.png)
...
![二哥的 Java 进阶之路：MRR 的缓冲区](assets/mysql-20250406122130.png)
...
![二哥的 Java 进阶之路：MRR 开启时的前后对比](assets/mysql-20250406124955.png)

[Wrap](https://javabetter.cn/gongju/warp.html) 也给出了对应的结果说明：

![二哥的 Java 进阶之路：MRR 的测试结果说明](assets/mysql-20250406125155.png)

也可以在 explain 中确认 MRR 的使用情况。

![二哥的 Java 进阶之路：使用聚簇索引时触发了 MRR](assets/mysql-20250406141028.png)
...
![技术派如何写到简历上模板](assets/mysql-20250406142139.png)
...
![Yxh_blogs：联合索引](assets/mysql-20250407152038.png)
...
![好奇的7号：联合索引](assets/mysql-20250407155043.png)
...
![mutest：联合索引](assets/mysql-20250407160853.png)
...
![球友拿到了腾讯天美的后台开发 offer](assets/mysql-20250408105425.png)

### 49.🌟覆盖索引了解吗？

覆盖索引指的是：查询所需的字段全部都在索引中，不需要回表，从索引页就能直接返回结果。

![Brand：覆盖索引](assets/mysql-20250408110120.png)
...
| WHERE B = 2 AND C = 3           | ❌ 否           | 无左侧列，索引失效              |
| WHERE A = 1 AND C = 3           | ⚠️ 部分生效     | 仅用 A 列，C 列无法利用索引优化 |
| WHERE B = 1 AND A = 2 AND C = 3 | ❌ 否           | 顺序不对，无左侧列              |
...
![三分恶面渣逆袭：联合索引](assets/mysql-e348203c-f00a-42a4-a745-b219d98ea435.jpg)
...
![二哥的Java 进阶之路：最左前缀匹配的差异](assets/mysql-20241105120556.png)
...
![二哥的 Java 进阶之路：explain中也可以明确看出来用了索引](assets/mysql-20241115153445.png)

第二条 SQL 语句由于未使用最左前缀中的 a，会触发全表扫描。

![二哥的 Java 进阶之路：rows 为 10 行，说明全表扫描了](assets/mysql-20241115153552.png)

第三条 SQL 语句在范围条件 `a > 2` 之后，索引后会停止匹配，b = 2 的条件需要额外过滤。

![二哥的 Java 进阶之路：rows 为 9 行说明的确走索引了，但还需要额外过滤](assets/mysql-20241115153636.png)
...
![二哥的 Java 进阶之路：验证是否走联合索引](assets/mysql-20240315140807.png)
...
![二哥的 Java 进阶之路：bc没有命中联合索引](assets/mysql-20240408092425.png)
...
![二哥的 Java 进阶之路：c不会命中联合索引](assets/mysql-20240408092646.png)
...
![二哥的java 进阶之路：联合索引前缀通配符](assets/mysql-20241104212447.png)

`type: ref` 表示使用索引查找匹配某个值的所有行。

![二哥的java 进阶之路：6 行数据](assets/mysql-20241104212743.png)

如果是后缀通配符，如 `age = 18 and name LIKE 'xxx%'`，MySQL 会直接使用联合索引 age_name 找到所有符合条件的行。

![二哥的java 进阶之路：联合索引后缀通配符](assets/mysql-20241104213135.png)
...
![Echo Blog：索引下推](assets/mysql-20250408150326.png)
...
![三分恶面渣逆袭：没有使用 ICP](assets/mysql-c58f59e0-850b-4dfd-8129-2dfc51cf4768.jpg)

启用 ICP 后，InnoDB 会通过联合索引直接筛选出符合条件的主键 ID（`name like '张%' and age=10`），然后再回表查询整行数据。

![三分恶面渣逆袭：使用 ICP](assets/mysql-a8525cf3-2d16-49a9-a7da-a19762ed16df.jpg)
...
![二哥的 Java 进阶之路：开启 ICP 和关闭 ICP 的查询语句](assets/mysql-20250408152401.png)

从结果中我们可以清楚地看到 ICP 的效果。ICP 开启时，Extra 列显示"Using index condition"，表明过滤条件被下推到存储引擎层。

ICP关闭时，Extra 列仅显示"Using where"，表明过滤条件在服务器层执行。

![二哥的 Java 进阶之路：开启 ICP前后的结果对比](assets/mysql-20250408152547.png)
...
![二哥的 Java 进阶之路：性能差距](assets/mysql-20250408153010.png)
...
![二哥的 Java 进阶之路：explain 和索引](assets/mysql-20240417092646.png)
...
![二哥的 Java 进阶之路：ac 会用到联合索引的一部分](assets/mysql-20240319131120.png)
...
![二哥的 Java 进阶之路：bc 无法命中索引](assets/mysql-20240319131245.png)
...
![二哥的 Java 进阶之路：acb 会命中索引](assets/mysql-20240319131306.png)
...
![球友拿到了腾讯和美团的 offer](assets/mysql-20250408155951.png)
...
![三分恶面渣逆袭：MySQL 中的锁](assets/mysql-a07e4525-ccc1-4287-aec5-ebf3f277857c.jpg)

---- 这部分是帮助大家理解 start，面试中可不背 ----

表锁：锁定整个表，资源开销小，加锁快，但并发度低，不会出现死锁；适合查询为主、少量更新的场景（如 MyISAM 引擎）。

![IServise：表级锁](assets/mysql-20250411093212.png)

再细分的话，有表共享读锁（S锁）：允许多个事务同时读，但阻塞写操作；表独占写锁（X锁）：独占表，阻塞其他事务的读写。

![Draven：共享锁和独占锁](assets/mysql-20250410121135.png)
...
![周二鸭：表锁](assets/mysql-20250409144701.png)
...
![周二鸭：行锁](assets/mysql-20250409150234.png)
...
![球友拿到了美团的暑期实习 offer](assets/mysql-20250409152600.png)

#### 说说记录锁吧？

记录锁是行锁最基本的表现形式，当我们使用唯一索引或者主键索引进行等值查询时，MySQL 会为该记录自动添加排他锁，禁止其他事务读取或者修改锁定记录。

![东平的笔记仓库：记录锁](assets/mysql-20250410114455.png)
...
![X1aoHei：间隙锁](assets/mysql-20250410114820.png)
...
![三分恶面渣逆袭：间隙锁](assets/mysql-d60f3a42-4b0f-4612-b7ad-65191fecb852.jpg)
...
![二哥的Java 进阶之路：间隙锁](assets/mysql-20241215095640.png)
...
![球友学院本拿到了滴滴的 SP](assets/mysql-%E7%AD%89%E4%B8%80%E6%89%8B%E7%BB%8F%E9%AA%8C%E8%B4%B4%E5%88%86%E4%BA%AB%EF%BC%8C%E5%8F%91%E4%B8%AA%E7%BA%A2%E5%8C%85%EF%BC%8C%E5%93%88%E5%93%88.png)
...
![三分恶面渣逆袭：意向锁](assets/mysql-31f7f49c-1e5a-4d42-b8b3-e022b3ba82ae.jpg)
...
![IServise：意向锁](assets/mysql-20250411093326.png)
...
![球友给面渣逆袭上了口碑](assets/mysql-%E4%B8%BB%E8%A6%81%E6%88%91%E7%9C%8B%E7%9A%84%E5%87%A0%E4%B8%AA%E5%85%AB%E8%82%A1%E9%83%BD%E6%98%AF%E7%BB%93%E5%90%88%E8%B5%B7%E6%9D%A5%E7%9A%84.png)

### 58.🌟MySQL的乐观锁和悲观锁了解吗？

悲观锁是一种"先上锁再操作"的保守策略，它假设数据被外界访问时必然会产生冲突，因此在数据处理过程中全程加锁，保证同一时间只有一个线程可以访问数据。

![牧小农：悲观锁](assets/mysql-20250411092155.png)

MySQL 中的行锁和表锁都是悲观锁。

![牧小农：悲观锁的处理思路](assets/mysql-20250411092536.png)

乐观锁会假设并发操作不会总发生冲突，属于小概率事件，因此不会在读取数据时加锁，而是在提交更新时才检查数据是否被其他事务修改过。

![牧小农：乐观锁](assets/mysql-20250411092610.png)
...
![球友进入京东的 HR 面了](assets/mysql-20250411101319.png)
...
![draven.co：死锁的发生](assets/mysql-20250413095712.png)
...
![二哥的 Java 进阶之路：死锁](assets/mysql-20241201101426.png)

解决办法也很简单，先使用 `SHOW ENGINE INNODB STATUS\G;` 确认死锁的具体信息，然后调整资源的访问顺序。

![二哥的 Java 进阶之路：查看死锁](assets/mysql-20241201101704.png)
...
![北野新津：ACID](assets/mysql-20250413102346.png)
...
![二哥的 Java 进阶之路：读未提交下出现脏读](assets/mysql-20250413155703.png)
...
![二哥的 Java 进阶之路：读已提交可以解决脏读问题](assets/mysql-20250413160617.png)
...
![二哥的 Java 进阶之路：读已提交会出现不可重复读的问题](assets/mysql-20250413162654.png)
...
![二哥的 Java 进阶之路：可重复读级别解决不可重复读的问题](assets/mysql-20250413162908.png)
...
![二哥的 Java 进阶之路：可重复读级别下可能出现幻读](assets/mysql-20250413171035.png)
...
![二哥的 Java 进阶之路：串行化隔离级别下不会出现幻读问题](assets/mysql-20250413171627.png)
...
![Mayank Sharma：可持久化](assets/mysql-20250413172141.png)
...
![球友的彩虹屁真的太猛了](assets/mysql-%E4%BA%8C%E5%93%A5%E6%82%A8%E5%A5%BD%EF%BC%8C%E9%BA%BB%E7%83%A6%E6%82%A8%E5%B8%AE%E5%BF%99%E6%94%B9%E4%B8%80%E4%B8%8B%E7%AE%80%E5%8E%86%EF%BC%8C%E4%BA%8C%E5%93%A5%E7%9C%9F%E6%98%AF%E9%85%8D%E4%BA%AB%E5%A4%AA%E5%BA%99%E7%9A%84%E7%94%B7%E4%BA%BA.png)
...
![二哥的 Java 进阶之路：ACID 的保证机制](assets/mysql-20230919103025.png)

#### 详细说说如何保证原子性？

事务对数据进行修改前，会记录一份快照到 Undo Log，如果事务中有任何一步执行失败，系统会读取 Undo Log 将所有操作回滚，恢复到事务开始前的状态，从而保证事务要么全部成功，要么全部失败。

![小许 code：undo log保证原子性](assets/mysql-20250414121841.png)
...
![小许 code：redo log 的 WAL，Write-Ahead Logging](assets/mysql-20250414154202.png)
...
![BookSea：Doublewrite](assets/mysql-20250414154539.png)
...
![一树一溪：2PC](assets/mysql-20250414155206.png)
...
![小许 code：Checkpoint](assets/mysql-20250414154331.png)
...
![阿里云社区：临键锁](assets/mysql-20250414162829.png)
...
![小余哥：ReadView](assets/mysql-20250414171111.png)
...
![二哥的 Java 进阶之路：@@autocommit](assets/mysql-20250414171920.png)
...
![昨天有球友发帖说拿到了字节、美团的暑期实习 offer](assets/mysql-20250414172446.png)

### 62.🌟事务的隔离级别有哪些？

隔离级别定义了一个事务可能受其他事务影响的程度，MySQL 支持四种隔离级别，分别是：读未提交、读已提交、可重复读和串行化。

![draven.co：事务的四个隔离级别](assets/mysql-20250413100533.png)
...
![易尘埃：读未提交](assets/mysql-20250415152542.png)

#### 什么是读已提交？

读已提交避免了脏读，但可能会出现不可重复读，即同一事务内多次读取同一数据结果会不同，因为其他事务提交的修改，对当前事务是可见的。

![易尘埃：读已提交](assets/mysql-20250415152926.png)
...
![易尘埃：可重复读](assets/mysql-20250415153434.png)
...
![易尘埃：串行化](assets/mysql-20250415153614.png)
...
![二哥的 Java 进阶之路：快照读和当前读的差别](assets/mysql-20250415162326.png)
...
![allaroundjava：Read uncommitted](assets/mysql-20250416112357.png)

读已提交会在更新数据前加行级排他锁，不允许其他事务写入或者读取未提交的数据，也就意味着事务2 不能在事务 1 提交之前读取到事务1 修改的数据，从而解决脏读的问题。

![allaroundjava：Read committed](assets/mysql-20250416114215.png)
...
![allaroundjava：Repeatable read](assets/mysql-20250416115217.png)
...
![球友拿到了京东的暑期实习 offer](assets/mysql-20250417215132.png)

### 64.🌟请详细说说幻读呢？

幻读是指在同一个事务中，多次执行相同的范围查询，结果却不同。这种现象通常发生在其他事务在两次查询之间插入或删除了符合当前查询条件的数据。

![Jenny：Phantom read](assets/mysql-20250417222847.png)
...
![二哥的 Java 进阶之路：可以发现产生幻读了](assets/mysql-20250417222448.png)
...
![二哥的 Java 进阶之路：临键锁能防止幻读](assets/mysql-20250417223640.png)
...
![转转技术：临键锁](assets/mysql-20250418102139.png)
...
![二哥的 Java 进阶之路：只用快照读](assets/mysql-20250417224334.png)
...
![Keep It Simple：Read View](assets/mysql-20250418103117.png)
...
![溪水静幽：当前读](assets/mysql-20250418102600.png)
...
![爱吃鱼饼的猫：快照读](assets/mysql-20250418105347.png)
...
![球友拿到了蚂蚁暑期实习 offer](assets/mysql-20250418105609.png)

### 65.🌟MVCC 了解吗？

MVCC 指的是多版本并发控制，每次修改数据时，都会生成一个新的版本，而不是直接在原有数据上进行修改。并且每个事务只能看到在它开始之前已经提交的数据版本。

![天瑕：undo log 版本链和 ReadView](assets/mysql-20241215073837.png)
...
![三分恶面渣逆袭：版本链](assets/mysql-765b3d83-14eb-4b56-8940-9d60bfaf1737.jpg)

#### 请详细说说什么是ReadView？

ReadView 是 InnoDB 为每个事务创建的一份“可见性视图”，用于判断在执行快照读时，哪些数据版本是当前这个事务可以看到的，哪些不能看到。

![二哥的 Java 进阶之路：ReadView](assets/mysql-20240415093703.png)
...
![二哥的 Java 进阶之路：ReadView判断规则](assets/mysql-20240415094939.png)
...
![小许 code：可见性匹配规则](assets/mysql-20250419162341.png)
...
![程序员x：readview 在可重复读和读已提交下的不同](assets/mysql-20250419163740.png)
...
![球友拿到了拼多多的 offer](assets/mysql-20250419164013.png)
...
![三分恶面渣逆袭：读写分离](assets/mysql-31df767c-db05-4de4-a05b-a45bcf76c1bf.jpg)
...
![轻风博客：主从同步](assets/mysql-20250420153404.png)
...
![三分恶面渣逆袭：业务代码封装](assets/mysql-771eb01f-3f1a-4437-8e1b-affe4de36ec3.jpg)

中等项目是通过 Spring + 多数据源插件、AOP 注解自动路由；

大型系统通常使用中间件，如 ShardingSphere、MyCat，支持自动路由、负载均衡、故障转移等功能。

![三分恶面渣逆袭：数据库中间件](assets/mysql-f2313613-25bd-4065-8f63-969a4b5757a7.jpg)
...
![鲲鹏：Mycat for MySQL 读写分离](assets/mysql-20250420160540.png)
...
![三分恶面渣逆袭：主从复制](assets/mysql-1bfbfcb5-2392-4f98-be1b-a66204da09e5.jpg)
...
![骏马金龙：半同步复制](assets/mysql-20250420164609.png)
...
![球友已经把星球安利给了同门](assets/mysql-20250420170230.png)

### 70.🌟你们一般是怎么分库的呢？

分库的策略有两种，第一种是垂直分库：按照业务模块将不同的表拆分到不同的库中，比如说用户、登录、权限等表放在用户库中，商品、分类、库存放在商品库中，优惠券、满减、秒杀放在活动库中。

![三分恶面渣逆袭：垂直分库](assets/mysql-2a43af18-617b-4502-b66a-894c2ff4c6c3.jpg)

第二种是水平分库：按照一定的策略将一个表中的数据拆分到多个库中，比如哈希分片和范围分片，对用户 id 进行取模运算或者范围划分，将数据分散到不同的库中。

![三分恶面渣逆袭：水平分库](assets/mysql-debe0fb1-d7f7-4ef2-8c99-13c9377138b6.jpg)
...
![三分恶面渣逆袭：表拆分](assets/mysql-7cba6ce0-c8bb-4f51-9c3b-e5a44e724c79.jpg)
...
![二哥的 Java 进阶之路：文章和详情垂直分表](assets/mysql-20250422152008.png)
...
![三分恶面渣逆袭：范围分片](assets/mysql-b3882ca3-1d04-44e2-9015-7e6c867255a0.jpg)
...
![三分恶面渣逆袭：Hash 分片](assets/mysql-e01e7757-c337-48c8-95db-2f7cfd2bc036.jpg)
...
![三分恶面渣逆袭：配置路由](assets/mysql-fcd34332-d38d-455a-875d-d4afd37cac72.jpg)
...
![拿到蚂蚁 offer 的球友对星球口碑+1 了](assets/mysql-20250422153403.png)

### 73.不停机扩容怎么实现？

第一个阶段：新旧库同时写入，确保数据实时同步；可以借助消息队列实现异步补偿，幂等避免重复写入。读操作仍然走旧库。

![三分恶面渣逆袭：数据同步和校验](assets/mysql-2d4d94c9-e816-47fc-93dd-a835b1318099.jpg)
...
![三分恶面渣逆袭：下线旧库](assets/mysql-a122d6d5-fff2-4ccd-8ddb-a9282eb2e2da.jpg)
...
![AWS：Sharding-JDBC](assets/mysql-20241207120214.png)

②、Mycat 是由阿里巴巴的一款产品 Cobar 衍生而来，可以把它看作一个数据库代理。

![piwenfei：mycat](assets/mysql-20241207121845.png)
...
![PmHub 项目中 Seata](assets/mysql-20250423110330.png)
...
![京东的 JD-hotkey](assets/mysql-20250423111247.png)
...
![技术派：自定义雪花算法算法](assets/mysql-20241223150915.png)
...
![技术派：雪花算法](assets/mysql-20241223150351.png)
...
![vivo 暑期实习 offer](assets/mysql-20250423111757.png)
...
![截图来自MySQL官网：由腾讯游戏 DBA 团队贡献](assets/mysql-20250424114631.png)

### 78.MySQL 导致 cpu 飙升的话，要怎么处理呢？

我通常先通过 top 命令确认是否是 mysqld 的进程占用。

![top -pid $(pgrep mysqld)](assets/mysql-20250424120022.png)
...
![球友拿到小鹏汽车的测试 offer](assets/mysql-20250424120308.png)
...
![二哥的 Java 进阶之路：找出age=18的记录](assets/mysql-20240410105325.png)
...
![二哥的 Java 进阶之路：找出年龄大于 18 的人](assets/mysql-20240410105512.png)
...
![二哥的 Java 进阶之路：排名前两名的学生](assets/mysql-20240410105951.png)
...
![二哥的 Java 进阶之路：先索引再插入](assets/mysql-20240412083019.png)
...
![二哥的 Java 进阶之路：先插入再索引](assets/mysql-20240412083312.png)
...
![家长对星球的认可](assets/mysql-%E8%80%81%E5%B8%88%E6%82%A8%E5%A5%BD%EF%BC%8C%E4%BB%8A%E5%A4%A9%E5%AD%A9%E5%AD%90%E5%92%8C%E6%88%91%E9%80%9A%E8%AF%9D%E8%AF%AD%E6%B0%94%E9%83%BD%E7%B2%BE%E7%A5%9E%E8%B5%B7%E6%9D%A5.png)
...
![二哥的 Java 进阶之路：排名前十](assets/mysql-20240423113508.png)
...
![二哥的 Java 进阶之路：窗口函数](assets/mysql-20250426150253.png)
...
![球友拿到蚂蚁国际的 offer 发来的感谢信](assets/mysql-20250427102433.png)

---

整整两个月，面渣逆袭 MySQL 篇第二版终于整理完了，这一版几乎可以说是重写了，每天耗费了大量的精力在上面，可以说是改头换面，有一种士别俩月，当刮目相看的感觉。

![Java 基础篇、集合框架篇、JVM 篇、并发编程篇、MySQL 篇](assets/mysql-20250427104640.png)
...
![面渣逆袭MySQL篇：epub 版本](assets/mysql-20250427104017.png)

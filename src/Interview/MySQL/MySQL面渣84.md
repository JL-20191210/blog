...
:star:转自[博客 | 二哥的Java进阶之路](https://javabetter.cn/blog.html) 感谢二哥救我:dog:命
...
![MySQL 8.3 最新版本](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311126086.png)
...
假设:star2:**要查询所有用户及他们的订单，即使用户没有下单**，就可以使用左连接：
...
![MySQL 内连接、左连接、右连接-来源菜鸟教程](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311126554.jpeg)
...
![技术派实战项目：内连接的结果](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311650851.png)
...
![技术派实战项目：做连接查询结果](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311650164.png)
...
![技术派实战项目：右连接查询结果](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311650491.png)

### 3.说一下数据库的三大范式？

![三分恶面渣逆袭：数据库三范式](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311650755.jpeg)

**第一范式(1NF)：数据库表的每一列都是不可分割的原子项。**比如说用户地址，应该拆分成省、市、区、详细地址等 4 个字段。

![Ruthless：第一范式](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311651678.png)

第二范式，要求表中的**每一列都和主键直接相关**。比如在订单表中，商品名称、单位、商品价格等字段应该拆分到商品表中。

![不符合第二范式](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311652046.png)

然后新建一个订单商品关联表，用订单编号和商品编号进行关联就好了。

![Ruthless：订单商品关联表](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311652462.png)

第三范式，**非主键列应该只依赖于主键列**。比如说在设计订单信息表的时候，可以把客户名称、所属公司、联系方式等信息拆分到客户信息表中，然后在订单信息表中用客户编号进行关联。

![Ruthless：第三范式](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503311653818.png)
...
![三分恶面渣逆袭：varchar和 char](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312233811.jpeg)
...
![三分恶面渣逆袭：DATETIME 和 TIMESTAMP](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312233952.jpeg)

另外，DATETIME 的默认值为 null，占用 8 个字节；TIMESTAMP 的默认值为当前时间——CURRENT_TIMESTAMP，占 4 个字节，实际开发中更常用，因为可以自动更新。

![二哥的 Java 进阶之路：更新时不用 set 更新时间](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312233878.png)

### 7.in 和 exists 的区别？

当使用 IN 时，MySQL 会首先执行子查询，然后将子查询的结果集用于外部查询的条件。这意味着子查询的结果集需要**全部加载到内存**中。
...
`IN` 适用于子查询**结果集较小**的情况。如果子查询返回大量数据，`IN` 的性能可能会下降，因为它需要将整个结果集加载到内存。

而 EXISTS 适用于**子查询结果集可能很大的情况**。由于 `EXISTS` 只需要判断子查询是否返回行，而**不需要加载整个结果集**，因此在某些情况下性能更好，特别是当子查询可以使用索引时。
...
```sql
-- 将表 mytable 的字符集改为 utf8mb4，并将其排序规则设置为 utf8mb4_unicode_ci
...
![二哥的 Java 进阶之路：查看 MySQL 的默认字符集](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312249456.png)
...
![二哥的 Java 进阶之路：count(\*)全表扫描](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312251506.png)
...
![二哥的 Java 进阶之路：修改t1主键](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312251184.png)

再来看一下 `select count(*)`，发现用了索引（MySQL 默认为给主键添加索引）。

![二哥的 Java 进阶之路：count(\*)走了索引](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312255604.png)

另外，MySQL 8.0 官方手册有明确说明，InnoDB 引擎对 `SELECT COUNT(*)` 和 `SELECT COUNT(1)` 的处理方式完全一致，性能并无差异。

![二哥的 Java 进阶之路：MySQL 8.0 官方手册](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312255903.png)
...
![博客园数据派：查询语句执行顺序](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312255355.png)
...
这个执行顺序与编写 SQL 语句的顺序不同，这也是为什么**有时候在 SELECT 子句中定义的别名不能在 WHERE 子句中使用得原因**，因为 WHERE 是在 SELECT 之前执行的。

#### LIMIT 为什么在最后执行？

`LIMIT` 必须在查询的最后执行，因为它依赖于其他操作（如筛选、排序等）完成后的结果。如果提前执行 `LIMIT`，可能会错过一些数据，导致查询结果不准确。

#### ORDER BY 为什么在 SELECT 之后执行？

它的作用是对最终返回的结果进行排序，而不是对数据源中的原始数据进行排序。`SELECT` 确定了需要哪些列和数据，而 `ORDER BY` 在这些列和数据都确定之后，才进行排序。

如果 ORDER BY 早于 SELECT 执行，计算 `COUNT(*)` 之类的聚合函数就会出问题。
...
![二哥的 Java 进阶之路：MySQL常用命令](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312304381.png)

MySQL 的常用命令主要包括**数据库操作命令、表操作命令、行数据 CRUD 命令、索引和约束的创建修改命令、用户和权限管理的命令、事务控制的命令**等。
...
`GRANT ALL PRIVILEGES ON database_name.table_name TO 'username'@'host';` 用于授予权限；`REVOKE ALL PRIVILEGES ON database_name.table_name FROM 'username'@'host';` 用于撤销权限；`DROP USER 'username'@'host';` 用于删除用户。
...
### :eyes:15.MySQL bin 目录下的可执行文件了解吗（补充）
...
![二哥的 Java 进阶之路：隐式转换](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312331103.png)
...
:one:第一个阶段，词法分析：拆解 SQL 语句，识别关键字、表名、列名等。
...
:two:第二个阶段，语法分析：检查 SQL 是否符合语法规则，并构建抽象语法树。
...
:three:第三个阶段，语义分析：检查表、列是否存在，进行权限验证等。
...
![三分恶面渣逆袭：MySQL的基础架构](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312337258.jpeg)
...
**![MySQL 的基础架构](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202504010010624.png)
...
![二哥的 Java 进阶之路：SQL 执行](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202503312351150.png)

第一步，客户端发送 SQL 查询语句到 MySQL 服务器。

第二步，MySQL 服务器的连接器开始处理这个请求，跟客户端建立连接、**获取权限**、管理连接。

第三步，解析器对 SQL 语句进行解析，检查语句是否符合 SQL 语法规则，确保数据库、表和列都是存在的，并处理 SQL 语句中的名称解析和**权限验证**。
...
![三分恶面渣逆袭：update 执行](https://felix-docs.oss-cn-beijing.aliyuncs.com/gitblogimg/202504010011024.jpeg)

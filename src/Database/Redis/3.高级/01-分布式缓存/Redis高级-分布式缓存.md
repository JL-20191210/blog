...
> Sentinel 是 Redis 的一个特殊运行模式，它本身也是 Redis 服务，只不过不负责存储数据，而是负责监控、自动故障转移和通知。生产中一般需要部署多个 Sentinel 实例来保证 Redis 集群的高可用。
...
> - 槽（slot）就像 **小区号**（一共 16384 个小区）。
>
> - key 就像 **住户**。
>
> - 很多住户（key）可能住在同一个小区（slot）。
>
> - 多个小区（slot）整体由同一个物业公司（Redis 节点）管理。
